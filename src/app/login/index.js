    ...
    <PageLayout>
      ...
      <SideLayout padding="medium">
        <form onSubmit={callbacks.onSubmit}>
          <h2>{translate.t("auth.title")}</h2>
          <Field label={translate.t("auth.login")} error={select.errors?.login}>
            <Input
              name="login"
              value={data.login}
          ...
          <Field
            label={translate.t("auth.password")}
          ...
          <Field>
            <button type="submit">{translate.t("auth.signIn")}</button>
          </Field>
        </form>
      </SideLayout>
    </PageLayout>
